...
describe("pipeEventsToNodeResponse — Gap 1 (error listener)", () => {
  test("aborts the run when the response emits an 'error' event", async () => {
    ...
    const events = (async function* () {
      ...
      yield { type: "agent:end", runId: "r1", result: { runId: "r1", content: "", messages: [], stopReason: "done", usage: { prompt_tokens: 0, completion_tokens: 0, total_tokens: 0 }, usageLog: [], generationIds: [] }, startedAt: 0, endedAt: 1, elapsedMs: 1 };
    })();
