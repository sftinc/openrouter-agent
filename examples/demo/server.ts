...
const calculator = new Tool({
	...
	display: {
		start: (args) => ({ title: `Calculating ${args.expression}` }),
		success: (args, output) => ({ title: `Calculated ${args.expression}`, content: output }),
		error: (args, error) => ({ title: `Calculator failed: ${args.expression}`, content: String(error) }),
	},
})
